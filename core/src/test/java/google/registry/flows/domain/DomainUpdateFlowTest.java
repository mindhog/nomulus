...
public class DomainUpdateFlowTest extends ResourceFlowTestCase<DomainUpdateFlow, DomainBase> {
    ...
    DomainBase domain =
        persistResource(
            newDomainBase(getUniqueIdFromCommand())
                ...
                .setContacts(
                    ImmutableSet.of(
                        DesignatedContact.create(Type.TECH, mak21Contact.createKey()),
                        DesignatedContact.create(Type.ADMIN, mak21Contact.createKey()),
                        DesignatedContact.create(Type.BILLING, mak21Contact.createKey())))
                .setRegistrant(mak21Contact.createKey())
    ...
    DomainBase domain =
        persistResource(
            newDomainBase(getUniqueIdFromCommand())
                ...
                .setContacts(
                    ImmutableSet.of(
                        DesignatedContact.create(Type.TECH, sh8013Contact.createKey()),
                        DesignatedContact.create(Type.ADMIN, unusedContact.createKey())))
                .setNameservers(ImmutableSet.of(host.createKey()))
                .build());
    historyEntryDomainCreate =
        persistResource(
  ...
  public void testSuccess_addAndRemoveLargeNumberOfNameserversAndContacts() throws Exception {
    ...
    for (int i = 0; i < 8; i++) {
      contactsBuilder.add(
          DesignatedContact.create(
              ...
              persistActiveContact(String.format("max_test_%d", i)).createKey()));
    }
    ...
    assertThat(tm().load(domain.getRegistrant()).get().getContactId()).isEqualTo("max_test_7");
    assertNoBillingEvents();
    assertDnsTasksEnqueued("example.tld");
  }
  ...
  public void testSuccess_registrantMovedToTechContact() throws Exception {
    ...
    persistResource(
        newDomainBase(getUniqueIdFromCommand())
            .asBuilder()
            .setRegistrant(sh8013.createKey())
            .build());
    clock.advanceOneMilli();
    runFlowAssertResponse(loadFile("generic_success_response.xml"));
  }
  ...
  public void testSuccess_multipleReferencesToSameContactRemoved() throws Exception {
    ...
    ContactResource sh8013 =
        loadByForeignKey(ContactResource.class, "sh8013", clock.nowUtc()).get();
    VKey<ContactResource> sh8013Key = sh8013.createKey();
    persistResource(
        newDomainBase(getUniqueIdFromCommand())
            .asBuilder()
  ...
  public void testFailure_removeAdmin() throws Exception {
    ...
    persistResource(
        newDomainBase(getUniqueIdFromCommand())
            ...
            .setContacts(
                ImmutableSet.of(
                    DesignatedContact.create(Type.ADMIN, sh8013Contact.createKey()),
                    DesignatedContact.create(Type.TECH, sh8013Contact.createKey())))
            .build());
  ...
  public void testSuccess_changeRegistrantWhitelisted() throws Exception {
    ...
    persistResource(
        Registry.get("tld")
            ...
            .build());
    runFlow();
    assertThat(tm().load(reloadResourceByForeignKey().getRegistrant()).get().getContactId())
        .isEqualTo("sh8013");
  }
  ...
  public void testSuccess_changeContactsAndRegistrant() throws Exception {
    ...
    reloadResourceByForeignKey()
        ...
        .forEach(
            contact -> {
              assertThat(tm().load(contact.getContactKey()).get().getContactId())
                  .isEqualTo("mak21");
            });
    assertThat(tm().load(reloadResourceByForeignKey().getRegistrant()).get().getContactId())
        .isEqualTo("mak21");
